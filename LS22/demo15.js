...
var m = new Map();
var o = {p: 'Hello World'};
m.set(o, 'content');    ////拿节点作为key值  一切皆是对象 可以拿任意的作为key
m.get(o); // "content"
m.has(o); // true
m.delete(o); // true
m.has(o); // false
...
var map = new Map();
map.set(['a'], 555);
map.get(['a']); // undefined  //get和set是三等判等  数组是引用数据类型  三等不等

//思考：
var map = new Map();
map.set('a', 555);
map.get('a'); // 555   //基本  三等相等

//
var map = new Map();
var k1 = ['a'];
var k2 = ['a'];   // var k1 = 'a';/var k1 = ['a'];   var k2 = new String('a');   111  222  三等不等 
map.set(k1, 111);
map.set(k2, 222);
map.get(k1); // 111
map.get(k2); // 222
...
// "no"
// "yes"
for (let item of map.entries()) {   //entries()就是遍历这个数组     把item换成 [key,value] 也可
