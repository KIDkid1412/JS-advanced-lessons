...
var point = {
    ...
    moveTo:function (x,y) {
        //内部嵌套函数
        function moveToX() {
            this.x = x;//this绑定到了全局 所以point里的x还是0
        }
        //内部嵌套函数
        function moveToY() {
            this.y = y;
...
var point = {
    ...
    moveTo:function (x,y) {
        //内部嵌套函数
        var moveToX = ()=>this.x=x;  //绑定到最外层 this.x=2
        //内部嵌套函数
...
//箭头函数导致this总是指向函数定义生效时所在的对象（最后）（本例是{id: 42}），所以输出的是42
//其实箭头函数里面没有自己的this，而是引用外层对象 的this
...
var f = foo.call({id: 1});   //无论怎样修改 绑定的都是最外层的对象 所以都是1
