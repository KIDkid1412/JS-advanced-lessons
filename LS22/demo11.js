...
let s = Symbol();//不能用new  new出来的是个对象 symbol是基本数据类型
typeof s;// "symbol"
...
//注意，Symbol函数的参数只是表示对当前Symbol值的描述（唯一性），因此相同参数的Symbol函数的返回值是不相等的。
// 没有参数的情况
var s1 = Symbol();
var s2 = Symbol();
s1 === s2 // false
...
const obj = {
    toString() {   //没有这个toString  会调用原型上的 返回Symbol([object Object])
        return 'abc';
    }
};
